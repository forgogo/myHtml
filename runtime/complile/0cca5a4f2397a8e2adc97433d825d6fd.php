        ...
        align-items: center;"> <svg t="1652968408167" class="icon" viewBox="0 0 1024 1024" version="1.1" p-id="3092"
          ...
          <li style="display: flex;
            ...
            text-align: center;">
              {pboot:nav num=15 parent=0}
                {pboot:if([nav:n]>3)}
              <li style="list-style: none;"> <a href="[nav:link]">[nav:name] </a></li>
              {/pboot:if}
...
{pboot:position separator=>> indextext=首页}
        ...
        <div class="met-img">
          <ul class="blocks-100 blocks-sm-2 blocks-md-2 blocks-xlg-2  met-page-ajax met-grid"  data-scale='0.7'>
            {pboot:list num=8 order=sorting}
            <li class="parent-slide shown page1"> <a href="[list:link]"​ title="[list:title]"> <span> <img data-original="[list:ico]" alt="[list:title]"> </span>
              <h4 class="on"> [list:title] </h4>
              </a> </li>
            {/pboot:list}
          </ul>
          <div> <div class="ab_pages">
  <ul>
